...
''' % outfile.hdr_comment)
    ...
    def _declare_device_inst(self, node, parent_driver):
        ...
        self.buf('\t.name\t\t= "%s",\n' % node.struct_name)
        if plat_name:
            self.buf('\t.platdata_\t\t= %s,\n' % plat_name)
        else:
            self.buf('\t.platdata_\t\t= &%s%s,\n' % (VAL_PREFIX, node.var_name))
        if parent_plat_name:
...
OUTPUT_FILES_COMMON = {
    ...
    'struct':
        OutputFile(Ftype.HEADER, 'include/generated/dt-structs-gen.h',
...
OUTPUT_FILES_NOINST = {
    'platdata':
        OutputFile(Ftype.SOURCE, 'tpl/dts/dt-platdata.c', DtbPlatdata.generate_plat,
